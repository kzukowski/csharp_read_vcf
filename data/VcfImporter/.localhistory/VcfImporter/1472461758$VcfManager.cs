...
namespace VcfImporter
{
    class VcfManager
    {
        ...
        {
            dbConnect.queriesTransaction(queries);
        }

        public void connect(string server, string datbase, string username, string password)
        {
            dbConnect = new DBConnect();
            dbConnect.Initialize(server, datbase, username, password);
        }

        public string generateInsert(string tableLocationAndName)
        {
        ...
        public List<string> generateInsert(string tableLocationAndName, int divideEveryColumn)//when database is too large it is necessary to send database in multiple packets
        {
